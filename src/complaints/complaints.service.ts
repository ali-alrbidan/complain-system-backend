...
// import { MonthlyStatistics } from './dto/interfaces';
...
//   async getMonthlyStatistics(months: number = 6): Promise<MonthlyStatistics[]> {
...
//       // عد الشكاوى المنشأة في هذا الشهر
//       const monthComplaints = complaints.filter(
//         (c) => c.createdAt >= monthStart && c.createdAt <= monthEnd,
//       );

//       // عد الشكاوى المنجزة في هذا الشهر
//       const monthCompleted = complaints.filter(
//         (c) =>
//           c.resolvedAt &&
//           c.resolvedAt >= monthStart &&
//           c.resolvedAt <= monthEnd &&
//           c.status === 'COMPLETED',
//       );
...
//         inProgress: monthComplaints.filter((c) => c.status === 'IN_PROGRESS')
//           .length,
//         new: monthComplaints.filter((c) => c.status === 'NEW').length,
//       });
//     }
...
//     // إنشاء إشعار للمواطن
//     const citizenNotification = await this.prisma.notification.create({
//       data: {
//         title: 'تم استلام شكواك',
//         message: `تم استلام شكواك برقم مرجعي: ${referenceNumber}`,
//         type: 'COMPLAINT_CREATED',
...
//     // إرسال الإشعار للمواطن عبر WebSocket
//     await this.notificationsGateway.sendNotificationToUser(
//       userId,
//       citizenNotification,
//     );
...
//       for (const employee of employees) {
//         const employeeNotification = await this.prisma.notification.create({
//           data: {
//             title: 'شكوى جديدة',
//             message: `شكوى جديدة برقم مرجعي: ${referenceNumber} من النوع: ${complaint.type}`,
//             type: 'NEW_COMPLAINT',
//             userId: employee.id,
//             complaintId: complaint.id,
//           },
...
//         // إرسال الإشعار للموظف عبر WebSocket
//         await this.notificationsGateway.sendNotificationToUser(
//           employee.id,
...
//   // تحديث طريقة update لإرسال الإشعارات في الوقت الفعلي
//   async update(
//     id: string,
//     updateComplaintDto: UpdateComplaintDto,
//     userId: string,
...
//   // تحديث طريقة addComment لإرسال الإشعارات في الوقت الفعلي
//   async addComment(
//     complaintId: string,
//     addCommentDto: AddCommentDto,
//     userId: string,
...
export class ComplaintsService {
  ...
  async findAll(query: QueryComplaintDto, userId: string, userRole: UserRole) {
    ...
    if (userRole === UserRole.CITIZEN) {
      ...
      if (!user?.departmentId) {
        // إذا كان departmentId = null، إرجاع مصفوفة فارغة
        return {
          data: [],
          meta: {
            total: 0,
  ...
  async lockComplaint(id: string, userId: string, userRole: UserRole) {
    ...
    // تحقق من القفل الحالي وانتهاء صلاحيته
    if (complaint.isLocked && complaint.lockedBy !== userId) {
      const lockExpiry = new Date(complaint.lockedAt!);
      lockExpiry.setMinutes(lockExpiry.getMinutes() + 30); // 30 دقيقة
      ...
      if (new Date() < lockExpiry) {
        ...
        throw new BadRequestException(
          `الشكوى محجوزة من قبل ${lockedUser?.name} حتى ${lockExpiry.toLocaleTimeString('ar-SA')}`,
        );
      }
      // القفل منتهي الصلاحية - يمكن قفله مجدداً
    }
    ...
    await this.prisma.complaintHistory.create({
      data: {
        action: 'LOCK',
        description: 'تم قفل الشكوى للمعالجة',
        complaintId: id,
        performedBy: userId,
      },
    });
  ...
  // فك القفل التلقائي للشكاوى المنتهية الصلاحية (كل 5 دقائق)
  @Cron(CronExpression.EVERY_5_MINUTES)
  async autoUnlockExpiredComplaints() {
    const thirtyMinutesAgo = new Date();
    thirtyMinutesAgo.setMinutes(thirtyMinutesAgo.getMinutes() - 30);

    const expiredLocks = await this.prisma.complaint.updateMany({
      where: {
        isLocked: true,
        lockedAt: {
          lt: thirtyMinutesAgo,
    ...
    if (expiredLocks.count > 0) {
      console.log(`تم فك قفل ${expiredLocks.count} شكوى منتهية الصلاحية`);
    }
  }

  // تجديد القفل (للموظف الذي قام بالقفل)
  async renewLock(id: string, userId: string) {
    const complaint = await this.prisma.complaint.findUnique({
      where: { id },
    });

    if (!complaint?.isLocked || complaint.lockedBy !== userId) {
      throw new ForbiddenException('لا يمكنك تجديد هذا القفل');
    }

    return await this.prisma.complaint.update({
      where: { id },
      data: {
        lockedAt: new Date(), // تجديد وقت القفل
      },
    });
  }
}
